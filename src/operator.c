...
void p2m(int iPart, int npart, cpuType center[3], cpuType M[]) 
{
	int n, idx;
	cpuType dx[3];
	cpuType m0, m1, m2, m3, m4, m5;
	for (n=0; n<NMULTI; n++) 
		M[n] = 0.0;
...
#else		
		dx[0] = (float) (INT2POS * part[idx].posi[0]) - center[0];
		dx[1] = (float) (INT2POS * part[idx].posi[1]) - center[1];
		dx[2] = (float) (INT2POS * part[idx].posi[2]) - center[2];
#endif
		m0 = MASSPART;
		m1 = -m0;
...
void m2m(cpuType dx, cpuType dy, cpuType dz, cpuType M[], cpuType tM[]) 
{
	cpuType toM[NMULTI];
...
void l2p( int ileaf )
{
	cpuType dx, dy, dz;
	int n;
	cpuType *Fn = leaf[ileaf].L;
	cpuType pot, pot0,pot1,pot2, pot3;
	cpuType Fx, Fy, Fz;
	for (n=leaf[ileaf].ipart; n<leaf[ileaf].ipart+leaf[ileaf].npart; n++) 
	{
		...
		dx = (float)(INT2POS * part[n].posi[0]) - leaf[ileaf].center[0];
		dy = (float)(INT2POS * part[n].posi[1]) - leaf[ileaf].center[1];
		dz = (float)(INT2POS * part[n].posi[2]) - leaf[ileaf].center[2];
...
void l2p_act( int ileaf ,int act )
{
	cpuType dx, dy, dz;
	int n;
	cpuType *Fn = leaf[ileaf].L;
...
void m2l(cpuType dx, cpuType dy, cpuType dz, cpuType M[], cpuType toL[]) 
{
	cpuType dr, r2;
	cpuType ir, ir2, ir3, ir4, ir5, ir6, ir7, ir8,ir9;
	cpuType rs2, irs2, irs3, irs, irs5, irs7;
	cpuType Fn[NMULTI];
	cpuType Dn[NMULTI];
	...
	cpuType drs = 0.5*dr/splitRadius;
	cpuType coeff = 1.0/sqrt(M_PI);
	cpuType facExp = exp(-drs*drs) * coeff;
	cpuType facErc = erfc(drs);
	...
	Fn[Y]  = M[0]*Dn[Y]  ;
	Fn[Z]  = M[0]*Dn[Z]  ;
//	if (abs(toL[X]) > 1.0e2 || abs(toL[Y]) > 1.0e2 || abs(toL[Z]) > 1.0e2 || abs(Fn[X]) > 1.0e2 || abs(Fn[Y]) > 1.0e2 || abs(Fn[Z]) > 1.0e2)
//		printf("m2l Dxyz (%e %e %e) r2 %e FnXYZ (%e %e %e) M0 %e DnXYZ (%e %e %e) fac1 %e -r3 %e facErc %e dr %e irs %e\n", dx, dy, dz, r2, Fn[X], Fn[Y], Fn[Z], M[0], Dn[X], Dn[Y], Dn[Z], fac[1], -ir3, facErc, dr, irs);
...
void l2l(cpuType dx, cpuType dy, cpuType dz, cpuType L[], cpuType toL[]) 
{
	...
	toL[Y] += L[Y];
	toL[Z] += L[Z];
//	if (abs(toL[1]) > 1.0e2 || abs(toL[2]) > 1.0e2 || abs(toL[3]) > 1.0e2 || abs(L[X]) > 1.0e2 || abs(L[Y]) > 1.0e2 || abs(L[Z]) > 1.0e2)
//		printf("l2l Dxyz (%e %e %e) L0 %e L (%e %e %e) toL (%e %e %e)\n", dx, dy, dz, L[0], L[X], L[Y], L[Z], toL[X], toL[Y], toL[Z]);
